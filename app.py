    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                ...
                return render_template('template_avatars.html', 
...
def submit():
    try:
        ...
        if response.status_code != 200:
            return render_template('template_submit.html', 
...
def videos():
    try:
        ...
        if response.status_code != 200:
            return render_template('template_videos.html', error=f"Failed to fetch videos from Heygen: {response.text}", videos=[])
        ...
        return render_template('template_videos.html', videos=videos)
        
    except Exception as e:
        print(f"Error in videos route: {str(e)}")
        return render_template('template_videos.html', error=str(e), videos=[])
